class EndScreen extends egret.Sprite{
   ...
   */
  private endUi(_highScore:number) {
  ...
  private init(){
    /* 获取本次游戏分数 */
    const getGameScore = Number(GameConfig.getGameScore());
    /* 本次游戏成绩与历史成绩对比 */
    const historyHighScore:number = getGameScore > UserData.personalHighScore ? getGameScore : UserData.personalHighScore;
    /* 设置最高游戏分数到开发数据域 */
